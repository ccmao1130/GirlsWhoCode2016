...
ball_size2 = random.randint(20, 80)
...
while not done:
	...
	ball_color2 = random.choice(possible_ball_colors) # This is outside because of variable scoping.

	pygame.draw.circle(screen, ball_color, [x_location, y_location], ball_size)
	pygame.draw.circle(screen, ball_color2, [x_location2, y_location2], ball_size2)
	...
	if x_location2 >= SCREEN_WIDTH - ball_size2 or x_location2 < ball_size2:
		x_speed2 = x_speed2 * -1

	if y_location2 >= SCREEN_HEIGHT - ball_size2 or y_location2 < ball_size2:
		y_speed2 = y_speed2 * -1
