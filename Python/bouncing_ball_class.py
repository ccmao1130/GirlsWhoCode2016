...
YELLOW = (255, 255, 51)
PURPLE = (204, 0, 204)
possible_ball_colors = [BLACK, WHITE, GREEN, RED, BLUE, GREY, YELLOW, PURPLE]

answer = False
while answer == False:
	num_balls = int(input("\nHow many balls would you like? Please don't go over 4000. "))
	if num_balls > 4000:
		answer = False
...
class BouncingBall(): 
	...
	def flashBounce(self, screen, colors, screen_width, screen_height):
		...
		ball_color = random.randint(0, 255), random.randint(0, 255), random.randint(0, 255) # This is outside because of variable scoping.
...
for x in range(num_balls):
	ball_list.append(BouncingBall(350, 250, random.randint(-10, 10), random.randint(-10, 10), random.randint(20, 80)))
...
while not done:
	...
	z = 0
	for y in ball_list:
		ball_list[z].flashBounce(screen, possible_ball_colors, SCREEN_WIDTH, SCREEN_HEIGHT)
		z += 1
